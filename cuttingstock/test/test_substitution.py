...
@pytest.mark.asyncio
async def test_main_algorithm_mid_multiple_out_of_stock_with_substitution():
    """
    Tests that the main algorithm correctly handles an out-of-stock situation
    by calling the handler and using multiple substitute materials, and that the
    substitutions is remembered for subsequent orders.
    ...
    # 1. Mock data
    mock_orders_df = pl.DataFrame({
        "order_number": ["ORDER-1", "ORDER-2","ORDER-3"], "order_idx": [0, 1,2],
        "front": ["KA125", "KA125","KA125"],
        "middle": ["KA125", "KA125","KA125"],
        "back": ["KA125", "KA125","KA125"],
        "width": [12, 12,12],
        "type": ["A", "B","C"],
    ...
    mock_roll_specs = {"85": {
        "KA125": {1: {"id": "R-KA125-1", "length": 1000}},
        "KA150": {1: {"id": "R-KA150-1", "length": 50000}, 2: {"id": "R-KA150-2", "length": 50}, 3: {"id": "R-KA150-3", "length": 50000}}
    }}
    ...
    mock_lp_solutions = [
        ...
        {
            "status": "Optimal",
            "variables": {"roll_w": 85, "demand_per_cut": 1000, "order_idx": 2},
            "material_specs": {"front": "KA125"},
        },
    ]
    ...
    with patch('cuttingstock.core.load_data'), \
        ...
        res0 = results[0]
        assert res0["order_number"] == "ORDER-1"
        assert res0["front"] == "KA125", "Material should be KA125 since it was enough"
        assert "R-KA125-1" in res0["front_roll_info"]

        res1 = results[1]
        assert res1["order_number"] == "ORDER-2"
        assert res1["front"] == "KA150", "Material should be substituted to KA150"
        assert "R-KA150-1" in res1["front_roll_info"]

        res2 = results[2]
        assert res2["order_number"] == "ORDER-3"
        assert res2["front"] == "KA150", "Substitution should be remembered"
        assert "R-KA150-1" in res2["front_roll_info"]

        assert len(results) == 3, "All orders should have been processed"
        assert len(handler_calls) == 1, "Handler should only be called once"
...
#                     2: {"id": "R-KA150-2", "length": 50000}},
#         "KA125": {1: {"id": "R-KA125-1", "length": 50000},
#                     2: {"id": "R-KA125-2", "length": 50},
#                     3: {"id": "R-KA125-3", "length": 50000}}
#     }}
...
#          patch('os.path.exists', return_value=False), \
#          patch('polars.DataFrame.write_database'), \
#          patch('cuttingstock.core.try_xgboost_solution', return_value=None), \
#          patch('cuttingstock.core.solve_linear_program', side_effect=mock_lp_solutions), \
#          patch('cuttingstock.core.greedy_nest', side_effect=mock_greedy_nest):
